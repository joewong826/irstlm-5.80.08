...
// 按倒序排列
int cmpFloatEntryDescendingOrder(const void* a,const void* b)
{
  if (*(float *)a > *(float*)b)
    return -1;
  else if (*(float *)a < *(float *)b)
...
void quantizeLanguageModelRespectively(inputfilestream& inp,std::ofstream* out,fstream& filebuff,const std::string tmpfile);
void quantizeLanguageModelUnified(inputfilestream& inp,std::ofstream* out,fstream& filebuff,const std::string tmpfile);

//----------------------------------------------------------------------
//  Global parameters (some are set in getArgs())
//----------------------------------------------------------------------

// 修改为0-254，一共255个值。最终的引擎使用的词库会+1，区间在1-255,0作为冒犯性词汇
int       k      = 255;//256   // number of centers
...
int main(int argc, char **argv)
{
  ...
  if(bUnified){
    quantizeLanguageModelUnified(inp,out,filebuff,tmpfile);
  } else {
    quantizeLanguageModelRespectively(inp,out,filebuff,tmpfile);
  }
...
void quantizeLanguageModelUnified(inputfilestream& inp,std::ofstream* out,fstream& filebuff,const std::string tmpfile){
  ...
  char line[MAX_LINE];
  unsigned long sumNum = 0;
  unsigned long dataPtsOffset = 0;
  //reset tempout file to start writing
  filebuff.seekg((streampos) 0);
  const char *words[MAXLEV + 3];
  ...
  while (inp.getline(line, MAX_LINE)){
    ...
    if (sscanf(line, "\\%d-grams", &Order) == 1){
      break;
    }
  ...
  dataPts = new DataItem[sumNum]; // allocate data
  dataPtsOffset = 0;
  ...
  do {
    ...
    if (backslash && sscanf(line, "\\%d-grams", &Order) == 1) {
      ...
      for (nPts = 0; nPts < N; nPts++) {
        ...
        MY_ASSERT(howmany == Order + 2 || howmany == Order + 1);
        sscanf(words[0], "%f", &logprob);
        dataPts[dataPtsOffset].pt = logprob; //exp(logprob * logten);
        dataPts[dataPtsOffset].idx = dataPtsOffset;
        dataPtsOffset++;
      }
      cerr << "-- End processing of " << Order << "-grams\n";
  ...
  cerr << "quantizing " << sumNum << " probabilities\n";

  centersP = new double[centers[Order]];
  mapP = new unsigned short[sumNum];

  //将每个条目出现的概率转换为0~255之间的词频
  ComputeCluster(centers[Order], centersP, sumNum, dataPts);


  for (unsigned int p = 0; p < sumNum; p++) {
/*    if(dataPts[p].code ==255) {
      cerr<<" 255\n";
    }*/
    mapP[dataPts[p].idx] = dataPts[p].code;
  }
  ...
  while (Order <=MaxOrder) {
    ...
    *out << centers[Order] << "\n";
    ...
    for (nPts = 0; nPts < numNgrams[Order]; nPts++) {
      ...
      *out << mapP[dataPtsOffset+nPts];
/*    if(mapP[dataPtsOffset+nPts] == 255){
      cerr<<" 255\n";
    }*/
  ...
  for (unsigned int i=1; i<N; i++) {
    ...
    if ((bintable[i].pt != bintable[i - 1].pt)) {
      if ((different % interval) == 0) {
        if ((currcode + 1) < centers && population[currcode] > 0) {
          currcode++;
        }
      }
      different++;
