...
OPTIONS:
       ...
       -g| --googleformat )    Input corpus is Googleformat 
...
while [ "$1" != "" ]; do
    case $1 in
        ...
        -k | --thread )    	shift; THREAD=$1;
                                ;;
        -g | --googleformat )   shift; GOOGLE_FORMAT=$1;
				;;
	-y | --year )     shift; YEAR=$1;
				;;
...
ngtbatch=${TOOL_SCRIPTS}/ngtbatch.sh
split_dict=${TOOL_SCRIPTS}/split-dict.pl
...
###############################
#工具分为5步
#第一步：格式化语料文件，给语料断句，添加<s></s>标签[google格式时，此步忽略]
...
then
	...
	then
		...
		echo "google format，pass this step."
	fi
...
then
	...
	${ngtbatch}  -L ${NAME}  -l ${LOCALE} -y ${YEAR} -o 1 -c ${CORPUS_PATH} -k $THREAD  -r ${NAME}.dict.gz &&

	$gunzip -c  ${NAME}.dict.gz  > ${NAME}_count_original.txt
    fi &&
    while [ 1 ];
    do
...
then
	...
	then
		...
		echo "Extracting dictionary from 1gram training corpus" &&
#		echo cp ${NAME}_count.txt $tmpdir/${NAME}.dictionary 
		 cp ${NAME}_count.txt ${NAME}.dictionary  &&
		 ${ngtbatch}  -L ${NAME}  -l ${LOCALE} -y ${YEAR} -o ${ORDER} -c ${CORPUS_PATH} -s ${NAME}.dictionary -k $THREAD  -r  ${NAME}.ngt.gz &&
		 ${build_sublm} $verbose  $smoothing  --size ${ORDER} --ngrams "$gunzip -c   ${NAME}.ngt.gz" -sublm lm  >> $logfile  2>&1 &&

		echo "Merging language models into ${NAME}.lm.gz" &&
		 ${merge_sublm} --size ${ORDER} --sublm lm -lm ${NAME}.ilm  >> $logfile  2>&1 #&
	fi
...
then
    ...
    do
        if [ ! -e "${NAME}.lm.gz" ];
	then 
		echo "error: file ${NAME}.lm.gz not exist.."; 
		exit 4; 
	fi

	echo ${prune_lm} --threshold=${PRUNE_THRESHOLD} ${NAME}.lm.gz ${NAME}.plm
	...
	#mv ${NAME}_not_tag.plm ${NAME}.plm
        ...
        python ${lm2golatindata} -l ${LOCALE} -v ${VERSION} -i ${NAME}.qlm -o ${NAME}_go.txt
